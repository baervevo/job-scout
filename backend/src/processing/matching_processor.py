...
class MatchingProcessor(Processor):
    def __init__(self, resume_data, **kwargs):
        self.resume_data = resume_data
        super().__init__(**kwargs)

    async def match_listings(self, listings: List[ListingKeywordData]) -> List[Match]:
        matches = []

        for listing in listings:
            match = await self.match_single_listing(listing)
            matches.append(match)

        return matches

    async def match_single_listing(self, listing: ListingKeywordData) -> Match:
        missing_keywords = self._find_missing_keywords(listing.keywords)
        similarity = self._calculate_cosine_similarity(listing.embedding)
        summary = self._generate_summary(listing.keywords)

        return Match(
            resume_id=self.resume_data.internal_id,
    ...
    def _calculate_cosine_similarity(self, embedding: List[float]) -> float:
        vec1 = torch.tensor(self.resume_data.embedding)
        vec2 = torch.tensor(embedding)
        cos_sim = torch.nn.functional.cosine_similarity(vec1.unsqueeze(0), vec2.unsqueeze(0))
        return cos_sim.item()

    def _find_missing_keywords(self, listing_keywords: List[str]) -> List[str]:
    ...
    def _generate_summary(self, listing_keywords: List[str]) -> str:
        resumes_kw_joined = ", ".join(self.resume_data.keywords)
        listings_kw_joined = ", ".join(listing_keywords)
